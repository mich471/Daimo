...
class Detailed extends \Lof\AdvancedReports\Controller\Adminhtml\AdvancedReports\Order
{
    /**
    ...
    {    
        ...
        $this->_initAction()->_setActiveMenu(
            'Lof_AdvancedReports::orderdetailed'
        )->_addBreadcrumb(
            __('Order Detailed'),
            __('Order Detailed')
        );
        $this->_registry->register('report_type', "detailed"); 
        $this->_registry->register('header_text', __("Orders Detailed"));
        $this->_view->getPage()->getConfig()->getTitle()->prepend(__('Order Detailed Report'));   
        
        $gridBlock = $this->_view->getLayout()->getBlock('adminhtml_advancedreport_order_detailed.grid');  
        $filterFormBlock = $this->_view->getLayout()->getBlock('grid.filter.form');
        $this->_initReportAction([$gridBlock, $filterFormBlock], 'detailed'); 
    ...
    public function _initReportAction($blocks, $report_type = "")
    {
        ...
        $margin = $this->_dataHelper->getConfig("charts_settings/margin");
        $margin = $margin?(int)$margin:0;
        $requestData['margin_profit'] = $margin;
        ...
        foreach ($blocks as $block) {
            if ($block) {
                ...
                $block->setMarginProfit($requestData['margin_profit']);
    ...
    protected function _isAllowed()
    {
        ...
        return $this->_authorization->isAllowed('Lof_AdvancedReports::orderdetailed');
    }
