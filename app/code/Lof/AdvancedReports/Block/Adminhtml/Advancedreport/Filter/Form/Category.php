 ...
 */
class Category extends \Lof\AdvancedReports\Block\Adminhtml\Advancedreport\Filter\Form
{
    static $arr = array();
    static $tmp = array();
    protected function _prepareForm()
    {
        ...
        $fieldset->addField('category_id', 'select', array(
            ...
            'note'      =>  __('Choose category to show reports.'),
        ));
    ...
    public function getTreeCategories($parentId,$level = 0, $caret = ' _ '){
        $allCats = $this->_objectManager->create('\Magento\Catalog\Model\Category')->getCollection()
        ...
        foreach($allCats as $category)
        {
            if(!isset(self::$tmp[$category->getId()])) {
                ...
                $arr[] = array("id"=>$category->getId(), "name"=>$prefix."(ID:".$category->getId().") ".$category->getName());
    ...
    public function toOptionArray() {
        $root_parent_id = 1;
        $root_parent_collection = $this->_objectManager->create('\Magento\Catalog\Model\Category')->getCollection()
                    ->addAttributeToSelect('*')
                    ->addAttributeToFilter('is_active','1')
        ...
        $arr2 = array("0" =>  __("-- Select A Category --"));
        if($arr) {
            foreach($arr as $item) {
                $arr2[$item['id']] = $item['name'];
            }
        }
        return $arr2;
    }
