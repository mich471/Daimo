...
class Autoimport extends \Magento\Backend\App\Action
{
    /**
    ...
    public function execute()
    {
        ...
        if ($importModel->isLatestInstalledVersion()) {
            $result = [
                'status'  => 'success',
                'error'   => __('GeoIP Database Update'),
                'alert'   => __(
                    "You have the latest %1 GeoIP Database available on our server.
                    Please check back later for the updates.",
                    $importModel->getDataLabel()
                ),
                'message' => $this->dataHelper->formatInstalledVersion($importModel->getInstalledVersion())
            ];
        } else {
            $result = $importModel->autoImportData();
        }
