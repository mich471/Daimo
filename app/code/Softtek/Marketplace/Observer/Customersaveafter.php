...
class Customersaveafter implements ObserverInterface
{
        ...
        try {
            ...
            if ($data) {
                ...
                if (isset($data['store_url'])) {
                    ...
                    if ($samurl != '' && $samurl != $customerId && $data['store_url'] != '') {
                        $this->messageManager
                ...
                if (isset($data['store_name']) && $data['store_name'] != '') {
                    ...
                    if (isset($data['entity_idpts']) && $data['entity_idpts'] != '') {
                        ...
                        $oldStatusId = $sellerdata->getOrigData('status_id');
                        if (isset($data['remove_seller']) && $data['remove_seller'] == 1) {
                            if ($data['remove_seller'] == 1) {
                                $data['status_id'] = 0;
                            }
                ...
                if (isset($data['entity_idpts']) && $data['entity_idpts'] != '') {
                    $sellerdata = $this->_initSeller($customerId, $data['entity_idpts'], $data['store_url']);
                    $sellerdata->setStatusId($data['status_id']);
                    if ($oldStatusId === false) {
                        $oldStatusId = $sellerdata->getOrigData('status_id');
                    }
                    $sellerdata->save();
                    if ($data['status_id'] == 3 && $data['status_id'] != $oldStatusId) {
                        $message = __('Your pre-registration has been approved. Enter store details for review.');
                        try {
                            $this->mailToSellerPreApproved($message, $customerObj);
                        } catch (\Magento\Framework\Exception\LocalizedException $e) {
                            $messagecatch = $e->getMessage();
                        } catch (\RuntimeException $e) {
                            $messagecatch = $e->getMessage();
                    ...
                    if ($data['status_id'] == 1 && $data['status_id'] != $oldStatusId) {
                        $message = __('Your registration has been approved.');
                        try {
                            $this->mailToSellerApproved($message, $customerObj);
                        } catch (\Magento\Framework\Exception\LocalizedException $e) {
                            $messagecatch = $e->getMessage();
                        } catch (\RuntimeException $e) {
                            $messagecatch = $e->getMessage();
     ...
     *
     */
    protected function saveattributeValue($value, $customerId)
    {
     ...
     *
     */
    protected function mailToSeller($message, $store_url, $customer)
    {
     ...
     *   PreApproved Email
     */
    protected function mailToSellerPreApproved($message, $customer)
    {
        $customerFirstName = $customer->getFirstName();
        $customerLastName = $customer->getLastName();
        $customerEmail = $customer->getEmail();
        $identifier    = $this->scopeConfig->getValue( 'purpletree_marketplace/general/seller_preapproved_email', \Magento\Store\Model\ScopeInterface::SCOPE_STORE);
        try {
            $emailTemplateVariables = [];
            $emailTemplateVariables['name'] = $customerFirstName;
            $emailTemplateVariables['sellername'] = $customerFirstName.' '.$customerLastName;
     ...
     *   Approved Email
     */
    protected function mailToSellerApproved($message, $customer)
    {
        $customerFirstName = $customer->getFirstName();
        $customerLastName = $customer->getLastName();
        $customerEmail = $customer->getEmail();
        $identifier    = $this->scopeConfig->getValue( 'purpletree_marketplace/general/seller_approved_email', \Magento\Store\Model\ScopeInterface::SCOPE_STORE);
        try {
            $emailTemplateVariables = [];
            $emailTemplateVariables['name'] = $customerFirstName;
            $emailTemplateVariables['sellername'] = $customerFirstName.' '.$customerLastName;
    ...
    protected function mailToAdmin($message, $store_url, $customer)
    {
         $customerFirstName = $customer->getFirstName();
         $customerLastName = $customer->getLastName();
         $customerEmail = $customer->getEmail();
