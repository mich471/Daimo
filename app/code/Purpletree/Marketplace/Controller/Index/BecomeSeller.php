...
class BecomeSeller extends Action
{
    ...
    public function execute()
    {
        ...
        $seller=$this->storeDetails->getStoreDetails($customerId);
        ...
        if (!$moduleEnable) {
         ...
         if (isset($seller) && is_array($seller) && isset($seller['status_id']) && $seller['status_id'] == 1) {
            return $this->_redirect('marketplace/index/sellers');
        }
        $this->coreRegistry->register('seller_id', $customerId);
        $this->_resultPage = $this->resultPageFactory->create();
        
        $this->_resultPage->getConfig()->getTitle()->set(__('Become Seller'));
